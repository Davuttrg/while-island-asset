...
function getChangedPrices(count, storage, base_price, amount) {
    ...
    if (amount) {
        {
            sale_price = Math.abs(Number(amount)) * Number(base_price) *
                (Math.pow(2, (1 - ((Math.log2(Number(count) / Number(storage)) + Math.log2((Number(count) - Number(amount) + 1) / Number(storage))) / 2))) - 1)
        }
    } else
...
export async function orderEvent(req, res) {
    ...
    orderObj['price'] = getChangedPrices(citySystemData.count, citySystemData.storage, productData.base_price, Number(orderType) ? amount : -amount).sale_price;
    ...
    userData.balance += Number(orderType) ? orderObj['price'] : - orderObj['price'];
    await user_collection.updateOne({ _id: ObjectId(userData._id) }, {
        $set: {
